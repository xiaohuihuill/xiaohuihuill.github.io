//console.log($("body").find("a").text());
//var bodyArr = []
//var chdren = $("body").find();
//var chdrenText = $("body").find().text()
//for(var i=0;i<chdren;i++){
//    if(chdrenText == ""){
//        continue
//    }else{
//        bodyArr.push(chdrenText)
//    }
//}
//console.log(bodyArr);
...
//  扩展栏地部向上按钮功能实现点击隐藏扩展栏
    ...
    function curSetEvent() {
        ...
        $(".head-ext-cpLr li").on("mouseleave", function () {
            $(".head-ext-cpLr li").removeClass("curr")
        })
    }
    ...
    function showExt(extStr, extLf) {
        ...
        lfSetEvent()
        curSetEvent()
        $(".head-ext-cpLf li:first").addClass("curr")
        ...
        for (var i = 0; i < obj.length; i++) {
            ...
            for (var j = 0; j < obj[i].length; j++) {
                if (j == 0) {
                    ...
                    arr1.push('<li>' + obj[i][j].name + '<a href="">' + obj[i][j].text + '</a></li>')
                }
            }
            unStr = ulStr + arr1.join("") + '</ul>';
            arr.push(unStr)
        }
        ele.html(arr.join(""))
    };
