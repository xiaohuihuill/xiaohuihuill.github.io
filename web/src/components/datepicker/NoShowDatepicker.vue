 ...
 <template>
    <div class="vue-datepicker" style="text-align:center">
        <div style="width:100%;height:36px;line-height:36px;margin:auto;font-size:20px;" @click.stop="show=!show">{{current | dateFormat}}</div>
        <div v-if="show" class="vue-datepicker-wrap">
            <div class="vue-datepicker-header">
                ...
                <span @click.stop="selectYear=!selectYear" class="vue-datepicker-header-btn vue-datepicker-header-btn-day">
            ...
            <div class="vue-datepicker-content">
                <table>
                    ...
                    <tbody>
                        <tr v-for="week of list">
                            <td v-for="weekday of week" @click="pick(weekday,week)" :class="{
                                                                                                                                        'flag': weekday.flag, 
                                                                                                                                        'active': !weekday.flag && weekday.isSelect}">
                                {{weekday.text}}
                            </td>
                        </tr>
                    </tbody>
                ...
                <div v-if="selectYear" class="vue-date-picker-year-panel">
                    ...
                    <ul ref="month">
                        <li v-for="(m, $index) of months" @click.stop="pickMonth($index + 1)" :class="{'active': $index + 1 == select.month}">
                            {{m}}
                        </li>
                    </ul>
                </div>
...
export default {
    ...
    data() {
        return {
            ...
            current: "1517565936020", // 已选择的日期时间。时间搓
            select: {
                // 已选择的日期对象
                year: "",
                month: "",
    ...
    filters: {
        // 日期格式过滤器
        dateFormat(val) {
            let eal = parseInt(val);
            const date = new Date(eal);
    ...
    methods: {
        ...
        numberList(start, end, flag) {
            ...
            for (let i = start; i <= end; i++) {
                ...
                for (let j = 0; j < this.current.length; j++) {
                    let date = new Date(+this.current[j]),
                        year = date.getFullYear(),
                        month = date.getMonth() + 1,
                        day = date.getDate();
                    if (this.select.month == month && this.select.year == year && i == day) {
                        isSelect = true;
                    }
                }
        ...
        // 更改选中时间并向父组件派发事件
        complete() {
            const date = +new Date(this.select.year,
                this.select.month,
                this.select.date)
            this.current = date;
