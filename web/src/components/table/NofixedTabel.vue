                ...
                <el-table-column label="操作" width="280">
                    <template slot-scope="scope">
                        <div class="file-operation-other clearfix">
                            ...
                            <a class="blue fl" @click="synchronizedBroadcast(scope.row)">同步播放</a>
                            <a class="blue fl" @click="examineDevice(scope.row),dialogVisible = true">查看设备</a>
                            <a class="blue fl" @click="addDevice(scope.row)">添加设备</a>
                            <a class="blue fl" @click="ScreencaptureManage(scope.row)">截屏管理</a>
                            <a class="blue fl" @click="delete1(scope.row)">不可编辑删除</a>
                        </div>
                    </template>
                </el-table-column>
            </el-table>
...
export default {
    ...
    methods: {
        ...
        // 播放列表
        playList(row) {
            this.$router.push({ path: "/devgroup_mag/play_list/examine_play_list" })
        },
        // 远程控制
        remoteControl(row) {
